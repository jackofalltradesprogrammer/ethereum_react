...
const deploy = async () => {
  ...
  const result = await new web3.eth.Contract(
      JSON.parse(compiledFactory.interface)) // instance with ABI object (not JSON) from constructor Contract() either interacts or to create new
    .deploy({ data: compiledFactory.bytecode }) // it tells we need to deploy a contract (transaction object) with arguments
